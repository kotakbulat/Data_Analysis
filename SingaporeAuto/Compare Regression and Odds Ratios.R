...
library(tidyverse)
#setwd("D:/1_Asprak Analitika Data/P3/Data & R Script P3")
sg_auto <- read_csv("SingaporeAuto.csv")
sg_auto2<-sg_auto%>%
...
fm_zip <- zeroinfl(visits ~ ., data = dt, dist = "poisson")
summary(fm_zip)

#Pembandingan Model
fm<-list("ML-Pois" = fm_pois,"NB" = fm_nbin, "ZINB" = fm_zinb0, "ZIP" = fm_zip)
...
rbind(logLik = sapply(fm, function(x) round(logLik(x), digits = 0)),Df = sapply(fm, function(x) attr(logLik(x), "df")),AIC = sapply(fm, function(x) round(AIC(x))),BIC = sapply(fm,function(x) round(BIC(x))))
...
round(c("Obs" = sum(dt$visits < 1),
        ...
        "ZIP" = sum(predict(fm_zip, type = "prob")[,1])))
